...
export const useCartStore = create<CartStore>()(persist((set, get) => ({
  ...
  addItem: async (item) => { set({ isLoading: true }); try { const current = get().items; const existing = current.find((i) => i.variantId === item.variantId); const next = existing ? current.map((i) => i.variantId === item.variantId ? { ...i, quantity: i.quantity + item.quantity } : i) : [...current, { ...item, lineId: crypto.randomUUID() }]; set({ items: next, cartId: get().cartId ?? crypto.randomUUID(), checkoutUrl: null }); } finally { set({ isLoading: false }); } },
  updateQuantity: async (variantId, quantity) => { if (quantity <= 0) { await get().removeItem(variantId); return; } set({ isLoading: true }); try { set({ items: get().items.map((i) => i.variantId === variantId ? { ...i, quantity } : i), checkoutUrl: null }); } finally { set({ isLoading: false }); } },
  removeItem: async (variantId) => { set({ isLoading: true }); try { const next = get().items.filter((i) => i.variantId !== variantId); set({ items: next, cartId: next.length ? get().cartId : null, checkoutUrl: null }); } finally { set({ isLoading: false }); } },
  clearCart: () => set({ items: [], cartId: null, checkoutUrl: null }), getCheckoutUrl: () => null, syncCart: async () => undefined,
