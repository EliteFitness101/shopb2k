    ...
    <footer className="border-t border-border/60 bg-black/40">
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-12 md:grid-cols-4">
          ...
          {COLUMNS.map((c) => <div key={c.heading}><p className="text-xs uppercase tracking-[0.25em] text-gold">{c.heading}</p><ul className="mt-4 space-y-2 text-sm text-muted-foreground">{c.links.map((l) => <li key={l.label}>{"href" in l ? <a href={l.href} target="_blank" rel="noopener noreferrer" className="hover:text-foreground focus-visible:outline-2 focus-visible:outline-gold">{l.label}</a> : <Link to={l.to as any} className="hover:text-foreground focus-visible:outline-2 focus-visible:outline-gold">{l.label}</Link>}</li>)}</ul></div>)}
        </div>
        <div className="mt-14 border-t border-border/60 pt-8">
          <div className="flex flex-col gap-4 text-xs uppercase tracking-widest text-muted-foreground md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-5"><a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="hover:text-gold">Instagram</a><a href="https://x.com" target="_blank" rel="noopener noreferrer" className="hover:text-gold">X</a><a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="hover:text-gold">YouTube</a><CookiePreferencesLink /></div>
          </div>
          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-[10px] uppercase tracking-widest text-muted-foreground/80"><span>Secure checkout · Paystack</span><span>Analytics by consent</span><span>Privacy controls available</span><span>Member access protected</span></div>
        </div>
