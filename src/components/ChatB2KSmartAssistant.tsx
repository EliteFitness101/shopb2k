...
function routeIntent(pathname: string) {
  if (pathname.startsWith("/wellness")) return { label: "Wellness", action: "Find a wellness option", to: "/wellness" };
  if (pathname.startsWith("/shop") || pathname.startsWith("/product")) return { label: "Shop", action: "Help me choose", to: "/me" };
  if (pathname.startsWith("/programs")) return { label: "Programs", action: "Match me to a plan", to: "/me" };
  if (pathname.startsWith("/knowledge") || pathname.startsWith("/blog")) return { label: "Learn", action: "Turn this into my next step", to: "/me" };
  return { label: "ResoFit", action: "Start my personalized assessment", to: "/me" };
}
...
export function ChatB2KSmartAssistant() {
  ...
  useEffect(() => {
    const update = () => setKeyboardOpen(Boolean(window.visualViewport && window.visualViewport.height < window.innerHeight * 0.78));
    const vv = window.visualViewport;
    vv?.addEventListener("resize", update);
    window.addEventListener("focusin", (e) => setFocusedForm(Boolean((e.target as HTMLElement)?.matches?.("input,textarea,select,[contenteditable='true']"))));
    window.addEventListener("focusout", () => setTimeout(() => setFocusedForm(Boolean(document.activeElement?.matches?.("input,textarea,select,[contenteditable='true']"))), 0));
    update();
    return () => vv?.removeEventListener("resize", update);
  }, []);

  const intent = useMemo(() => routeIntent(pathname), [pathname]);
  const suppressed = keyboardOpen || focusedForm || pathname.includes("/checkout") || pathname.includes("/payment") || pathname === "/cart";
  const tipSuppressed = pathname === "/auth" || pathname.startsWith("/admin") || pathname.includes("/checkout") || pathname.includes("/payment") || pathname === "/cart";
      ...
      <div className="mx-auto flex max-w-7xl items-end justify-end px-4 sm:px-6">
        <div className="pointer-events-auto flex flex-col items-end gap-2">
            ...
            <div role="dialog" aria-label="ChatB2K assistant" className="w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-gold/20 bg-background/95 shadow-2xl backdrop-blur-2xl">
              <div className="border-b border-border/60 bg-black/30 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl border border-gold/30 bg-gold/10 text-gold"><Sparkles className="h-5 w-5" /></div><div><p className="font-medium">ChatB2K™</p><p className="text-[11px] text-muted-foreground">Personalized wellness guidance</p></div></div>
              ...
              <div className="space-y-3 p-4">
                ...
                <Link to={intent.to as never} onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-between rounded-2xl border border-gold/30 bg-gold/10 px-4 text-sm font-medium text-foreground transition hover:bg-gold/15 focus-visible:outline-2 focus-visible:outline-gold">
                  <span>{intent.action}</span><ArrowRight className="h-4 w-4 text-gold" />
                </Link>
          ...
          <button type="button" aria-label={open ? "Close ChatB2K assistant" : "Open ChatB2K assistant"} aria-expanded={open} onClick={() => setOpen((v) => !v)} className="group flex h-12 items-center gap-2 rounded-full border border-gold/30 bg-black/80 px-4 text-xs font-semibold uppercase tracking-widest text-foreground shadow-xl backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-gold/60 focus-visible:outline-2 focus-visible:outline-gold motion-reduce:transition-none">
            <MessageCircle className="h-4 w-4 text-gold" /><span className="hidden sm:inline">ChatB2K™</span><span className="sm:hidden">Help</span>
          </button>
        </div>
      </div>
    </div>
