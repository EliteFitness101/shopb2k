...
export const Route = createFileRoute("/personalize")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Take the ChatB2K Assessment and receive a personalized ResoFit pathway tailored to your goal, activity, and diet.",
      },
...
function recommendationFor(answers: Answers): Recommendation {
  const shopUrl = `${SHOP_URL}?goal=${encodeURIComponent(answers.goal)}`;
  if (answers.goal === "fat_loss") {
    return {
      title: "ResoFit Fat-Loss Pathway",
      summary: `Your protocol is matched to fat loss, ${answers.activity} activity and a ${answers.diet} diet. Start with the verified ResoFit 7-Day Reset offer, then continue into the appropriate pathway from the ResoFit shop.`,
      sku: "res-dig-reset",
      priceNGN: 1000,
      shopUrl,
      label: "Shop your fat-loss pathway",
    };
  }
  if (answers.goal === "reset") {
    return {
      title: "ResoFit 7-Day Reset",
      summary: `Your protocol is matched to a full reset with ${answers.activity} activity and a ${answers.diet} diet. Begin with the verified reset pathway in the ResoFit shop.`,
      sku: "res-dig-reset",
      priceNGN: 1000,
      shopUrl,
      label: "Start your reset",
    };
  }
  return {
    title: answers.goal === "muscle" ? "ResoFit Strength Pathway" : "ResoFit Energy & Wellness Pathway",
    summary: `Your protocol is matched to ${answers.goal}, ${answers.activity} activity and a ${answers.diet} diet. The ResoFit shop will show the current options for your goal.`,
    shopUrl,
...
function PersonalizePage() {
  ...
  const submit = (final: Answers) => {
    ...
    const next = recommendationFor(final);
  ...
  return (
    <section className="rounded-lg border border-border/60 bg-card/40 p-6 md:p-8">
      ...
      <div className="mt-6 grid gap-3">
        {options.map((o) => (
          <button key={o.value} type="button" disabled={loading} onClick={() => onSelect(o.value)} className={`group flex items-center justify-between rounded-md border px-5 py-4 text-left transition-colors ${value === o.value ? "border-gold bg-gold/10 text-foreground" : "border-border hover:border-gold/60 hover:bg-card"} disabled:opacity-50`}>
      ...
      <div className="mt-6 flex items-center justify-between">
        {onBack ? <button type="button" onClick={onBack} className="text-xs uppercase tracking-widest text-muted-foreground hover:text-foreground" disabled={loading}>← Back</button> : <span />}
        {loading && <span className="flex items-center gap-2 text-xs uppercase tracking-widest text-gold"><Loader2 className="h-3 w-3 animate-spin" /> Matching your protocol…</span>}
      </div>
    </section>
  );
}
...
function ResultView({ recommendation, answers, revealed, onReveal, onRestart }: { recommendation: Recommendation; answers: Answers; revealed: boolean; onReveal: () => void; onRestart: () => void }) {
  const copy = async () => {
    const text = [`ResoFit Protocol — ${recommendation.title}`, `Goal: ${answers.goal} · Activity: ${answers.activity} · Diet: ${answers.diet}`, "", recommendation.summary, recommendation.sku ? `Verified SKU: ${recommendation.sku}` : "", recommendation.shopUrl].filter(Boolean).join("\n");
  ...
  return (
    <article className="relative overflow-hidden rounded-xl border border-gold/40 bg-gradient-to-b from-black via-black to-black/80 p-6 text-foreground shadow-2xl shadow-gold/10 md:p-10">
      <div className="flex items-start justify-between gap-4">
        <div><p className="text-[10px] uppercase tracking-[0.3em] text-gold">CoachB2K Result</p><h2 className="mt-2 font-display text-3xl text-gold md:text-4xl">{recommendation.title}</h2></div>
      ...
      <div className="relative mt-6">
        <div className={`space-y-6 transition-all duration-500 ${revealed ? "blur-0" : "pointer-events-none select-none blur-md"}`} aria-hidden={!revealed}>
          ...
          <div className="rounded-lg border border-gold/30 bg-gold/5 p-5">
            ...
            {recommendation.sku && <p className="mt-3 text-xs text-muted-foreground">SKU: <span className="text-foreground">{recommendation.sku}</span>{recommendation.priceNGN ? ` · ₦${recommendation.priceNGN.toLocaleString("en-NG")}` : ""}</p>}
            <a href={recommendation.shopUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("checkout_start")} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-sm bg-gold px-5 py-3 text-xs font-semibold uppercase tracking-widest text-gold-foreground hover:bg-gold/90">
              {recommendation.label} <ArrowRight className="h-4 w-4" />
            </a>
          </div>
        </div>
        {!revealed && <button type="button" onClick={onReveal} className="absolute inset-0 flex flex-col items-center justify-center gap-3 rounded-md bg-black/30 backdrop-blur-sm">
          <span className="flex items-center gap-2 rounded-full border border-gold bg-gold px-6 py-3 text-xs font-semibold uppercase tracking-widest text-gold-foreground shadow-lg shadow-gold/30"><Sparkles className="h-3.5 w-3.5" /> Reveal my protocol</span>
          <span className="text-[11px] uppercase tracking-widest text-white/80">Curated for you</span>
        </button>}
      </div>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-border/40 pt-6">
        ...
        <button type="button" onClick={onRestart} className="text-xs uppercase tracking-widest text-muted-foreground hover:text-foreground">Retake assessment</button>
      </div>

      <a href={ELITE_WA_URL} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("whatsapp_click")} className="mt-8 flex items-center justify-between gap-4 rounded-lg border border-gold/40 bg-gradient-to-r from-gold/10 via-transparent to-gold/10 p-5 hover:from-gold/20 hover:to-gold/20">
        <div className="flex items-center gap-3"><Crown className="h-5 w-5 text-gold" /><div><p className="text-sm font-semibold text-foreground">Upgrade to Elite Coaching</p><p className="text-xs text-muted-foreground">1:1 with CoachB2K · custom programming · concierge access</p></div></div>
