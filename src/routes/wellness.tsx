...
const WELLNESS_HERO_POSTER = "https://ab2ttlkn9no0tuoa.public.blob.vercel-storage.com/buffer/assets/cover/resofit-cover-3.jpeg";
const WELLNESS_SOCIAL_COVER = WELLNESS_HERO_POSTER;
...
async function locator<T>(params: Record<string, string | number | undefined>) {
  ...
  if (!response.ok) throw new Error(`Wellness locator failed: ${response.status}`); return response.json() as Promise<T>;
}
...
export const Route = createFileRoute("/wellness")({
  head: () => ({ meta: [
    ...
    { property: "og:title", content: "ResoFit Wellness — Find Your Wellness Path" }, { property: "og:description", content: "One intelligent front door for wellness discovery powered by ChatB2K™." },
    { property: "og:type", content: "website" }, { property: "og:url", content: "https://resofit.fit/wellness" },
    { property: "og:image", content: WELLNESS_SOCIAL_COVER }, { property: "og:image:width", content: "1200" }, { property: "og:image:height", content: "630" },
    { name: "twitter:card", content: "summary_large_image" }, { name: "twitter:image", content: WELLNESS_SOCIAL_COVER },
  ], links: [{ rel: "canonical", href: "https://resofit.fit/wellness" }] }), component: WellnessHome,
});

function WellnessHome() {
  const [states, setStates] = useState<State[]>([]); const [query, setQuery] = useState(""); const [locationMessage, setLocationMessage] = useState(""); const [loading, setLoading] = useState(false);
  useEffect(() => { trackEvent("wellness_view"); locator<{ states: State[] }>({ action: "states" }).then((data) => setStates(data.states.filter((state) => state.status === "active"))).catch(() => setLocationMessage("Wellness discovery is temporarily unavailable.")); }, []);
  const locate = () => { if (!navigator.geolocation) { setLocationMessage("Location services are unavailable. You can choose a state below."); return; } setLoading(true); setLocationMessage("Resolving your location…"); navigator.geolocation.getCurrentPosition(({ coords }) => { trackEvent("wellness_location_detected"); window.location.href = `/wellness/states/cities/hubs/geo-locator?lat=${coords.latitude}&lng=${coords.longitude}`; }, () => { setLoading(false); setLocationMessage("Location permission was not granted. Choose a state instead."); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 300000 }); };
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><main>
    <section className="relative min-h-[72svh] overflow-hidden border-b border-border/60"><div className="absolute inset-0 bg-background"><img src={WELLNESS_HERO_POSTER} alt="" aria-hidden="true" width={1200} height={630} fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-cover opacity-60" onError={(e) => { e.currentTarget.src = heroImg; }} /><video className="absolute inset-0 h-full w-full object-cover opacity-70" src={WELLNESS_HERO_VIDEO} autoPlay muted loop playsInline preload="metadata" poster={WELLNESS_HERO_POSTER} aria-hidden="true" onError={(e) => { e.currentTarget.style.display = "none"; }} /><div className="absolute inset-0 bg-gradient-to-r from-background via-background/75 to-background/15" /><div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-background/35" /></div>
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-24 md:py-32"><div className="max-w-4xl"><p className="mb-5 text-xs uppercase tracking-[0.35em] text-gold">ResoFit™ Wellness Network</p><h1 className="font-display text-6xl leading-[0.92] sm:text-7xl md:text-8xl">What wellness<br /><span className="text-gradient-gold">do you need today?</span></h1><p className="mt-7 max-w-2xl text-lg leading-relaxed text-muted-foreground">Tell us what you need, where you are, or simply use your location. ResoFit connects your request to the verified wellness network available around you.</p></div>
        <form onSubmit={submitSearch} className="mt-10 max-w-3xl"><label htmlFor="wellness-search" className="sr-only">Search for a wellness service, location, need or goal</label><div className="flex items-center gap-3 border border-border bg-card/40 p-2 focus-within:border-gold"><Search className="ml-3 h-5 w-5 shrink-0 text-muted-foreground" /><input id="wellness-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search for a service, location, need or wellness goal…" className="min-w-0 flex-1 bg-transparent px-2 py-4 text-base outline-none placeholder:text-muted-foreground" /><button type="submit" className="inline-flex h-12 items-center gap-2 bg-gold px-5 text-xs font-semibold uppercase tracking-widest text-gold-foreground">Search <ArrowRight className="h-4 w-4" /></button></div></form>
