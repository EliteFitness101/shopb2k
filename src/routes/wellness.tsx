...
function WellnessHome() {
  ...
  useEffect(() => {
    ...
    locator<{ states: State[] }>({ action: "states" })
      .then((data) => setStates(data.states.filter((state) => state.status === "active")))
      .catch(() => setLocationMessage("Wellness discovery is temporarily unavailable."));
  }, []);

  const locate = () => {
    if (!navigator.geolocation) {
      setLocationMessage("Location services are unavailable. You can choose a state below.");
      return;
    }
    setLoading(true);
    setLocationMessage("Resolving your location…");
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        trackEvent("wellness_location_detected");
        window.location.href = `/wellness/states/cities/hubs/geo-locator?lat=${coords.latitude}&lng=${coords.longitude}`;
      },
      () => {
        setLoading(false);
        setLocationMessage("Location permission was not granted. Choose a state instead.");
      },
        ...
        <section className="relative overflow-hidden border-b border-border/60">
          <div className="mx-auto max-w-7xl px-6 py-24 md:py-32">
            <div className="max-w-4xl">
              ...
              <p className="mt-7 max-w-2xl text-lg leading-relaxed text-muted-foreground">Tell us what you need, where you are, or simply use your location. ResoFit connects your request to the verified wellness network available around you.</p>
            </div>

            <form onSubmit={submitSearch} className="mt-10 max-w-3xl">
              <label htmlFor="wellness-search" className="sr-only">Search for a wellness service, location, need or goal</label>
              <div className="flex items-center gap-3 border border-border bg-card/40 p-2 focus-within:border-gold">
                <Search className="ml-3 h-5 w-5 shrink-0 text-muted-foreground" />
                <input id="wellness-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search for a service, location, need or wellness goal…" className="min-w-0 flex-1 bg-transparent px-2 py-4 text-base outline-none placeholder:text-muted-foreground" />
                <button type="submit" className="inline-flex h-12 items-center gap-2 bg-gold px-5 text-xs font-semibold uppercase tracking-widest text-gold-foreground">Search <ArrowRight className="h-4 w-4" /></button>
              </div>
            </form>
