...
function MakaveliServices() {
  return <div className="min-h-screen bg-background"><SiteHeader/><main>
    <section className="border-b border-border/60 bg-gradient-to-b from-gold/[0.06] to-transparent"><div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
      ...
      <p className="mt-7 max-w-2xl text-lg leading-relaxed text-muted-foreground">Choose an experience, review what is included and book directly. Fixed-price services can be paid online; request-only services open a booking enquiry.</p>
    ...
    <section className="py-12"><div className="mx-auto max-w-7xl px-6"><div className="flex flex-wrap gap-2 border-b border-border/60 pb-6">{MAKAVELI_CATEGORIES.map(category => <a key={category} href={`#${category.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`} className="rounded-full border border-border px-4 py-2 text-[10px] uppercase tracking-widest text-muted-foreground transition-colors hover:border-gold hover:text-gold">{category}</a>)}</div>
      {MAKAVELI_CATEGORIES.map(category => { const services = MAKAVELI_SERVICES.filter(s => s.category === category); const id=category.toLowerCase().replace(/[^a-z0-9]+/g,"-"); return <section key={category} id={id} className="scroll-mt-24 py-12"><div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[.25em] text-gold">Catalogue</p><h2 className="mt-2 font-display text-3xl md:text-5xl">{category}</h2></div><span className="text-xs text-muted-foreground">{services.length} options</span></div><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map(service => {
        ...
        return <article key={service.slug} className={`group flex min-h-[440px] flex-col rounded-2xl border p-6 transition-all duration-300 hover:-translate-y-1 hover:border-gold/60 ${tierStyles[category] ?? "border-border/70 bg-card/30"}`}>
          <div className="flex min-h-6 items-start justify-between gap-4">{badges[service.slug] ? <span className="inline-flex items-center gap-1 rounded-full border border-gold/40 bg-gold/10 px-3 py-1 text-[9px] font-semibold uppercase tracking-widest text-gold"><Sparkles className="h-3 w-3"/>{badges[service.slug]}</span> : <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{isFuture ? "Coming soon" : isRequest ? "Request" : "Book & pay"}</span>}<span className="text-right font-display text-xl text-gold">{price}{service.priceMode === "from" && service.priceLabel?.includes("/person") ? <span className="ml-1 text-[10px] font-sans text-muted-foreground">/person</span> : null}</span></div>
          <div className="mt-5"><span className="text-[10px] uppercase tracking-widest text-muted-foreground">{category}</span><h3 className="mt-2 font-display text-2xl leading-tight">{service.name}</h3></div>
