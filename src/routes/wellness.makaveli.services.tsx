...
import { MAKAVELI_PRODUCTION_CATEGORIES, MAKAVELI_PRODUCTION_SERVICES, formatNGN } from "@/lib/makaveliProductionCatalog";

export const Route = createFileRoute("/wellness/makaveli/services")({
  head: () => ({ meta: [
    { title: "Makaveli Wellness Services & Pricelist — ResoFit" },
    { name: "description", content: "Makaveli Wellness Services catalogue with transparent pricing, service features and dynamic booking." },
  ], links: [{ rel: "canonical", href: "https://resofit.fit/wellness/makaveli/services" }] }),
  component: MakaveliServices,
});

const tierStyles: Record<string, string> = {
  "Start Here": "border-gold/70 bg-gold/[0.08] shadow-[0_0_40px_rgba(212,175,55,0.12)]",
...
const badges: Record<string, string> = {
  "reset-wellness-smoothie-ticket": "Start Here",
...
function MakaveliServices() {
  const [selectedCategory, setSelectedCategory] = useState(MAKAVELI_PRODUCTION_CATEGORIES[0] ?? "Core Wellness & Recovery");
  const services = useMemo(() => MAKAVELI_PRODUCTION_SERVICES.filter(service => service.category === selectedCategory), [selectedCategory]);
  const actualServiceCount = MAKAVELI_PRODUCTION_SERVICES.filter(service => service.slug !== "reset-wellness-smoothie-ticket").length;

  return <div className="min-h-screen bg-background"><SiteHeader/><main>
    <section className="border-b border-border/60 bg-gradient-to-b from-gold/[0.08] to-transparent"><div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
      <p className="text-xs uppercase tracking-[0.35em] text-gold">Makaveli Wellness Services · Aba</p>
      <h1 className="mt-4 max-w-5xl font-display text-5xl leading-[.95] md:text-8xl">Relax. Recover.<br/><span className="text-gradient-gold">Reconnect.</span></h1>
      <p className="mt-7 max-w-3xl text-lg leading-relaxed text-muted-foreground">{actualServiceCount} actual wellness services and programmes, plus the separate ₦1,000 Reset Wellness Smoothie + Ticket. Available services can be paid online; request-only and future services are clearly labelled.</p>
      <div className="mt-8 flex flex-wrap gap-3"><Link to="/wellness/makaveli/book" className="inline-flex items-center gap-2 bg-gold px-6 py-3 text-xs font-semibold uppercase tracking-widest text-gold-foreground">Book your experience <ArrowRight className="h-4 w-4"/></Link><a href="https://wa.me/2349032712393" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border border-border px-6 py-3 text-xs font-semibold uppercase tracking-widest"><MessageCircle className="h-4 w-4"/>WhatsApp</a></div>
    </div></section>

    <section className="py-10"><div className="mx-auto max-w-7xl px-6">
      <div className="mb-8 rounded-2xl border border-border/60 bg-card/20 p-3"><div className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Makaveli service categories">
        {MAKAVELI_PRODUCTION_CATEGORIES.map(category => <button key={category} type="button" role="tab" aria-selected={selectedCategory === category} onClick={() => setSelectedCategory(category)} className={`shrink-0 rounded-full border px-4 py-2 text-[10px] uppercase tracking-widest transition-colors ${selectedCategory === category ? "border-gold bg-gold/10 text-gold" : "border-border text-muted-foreground hover:border-gold hover:text-gold"}`}>{category}</button>)}
      </div></div>
      ...
      <section aria-labelledby="selected-category"><div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-[.25em] text-gold">Catalogue</p><h2 id="selected-category" className="mt-2 font-display text-3xl md:text-5xl">{selectedCategory}</h2></div><span className="text-xs text-muted-foreground">{services.length} options</span></div>
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map(service => {
          const isFuture = service.status === "coming_soon"; const isRequest = service.booking === "request";
          const price = service.priceMode === "fixed" && service.price !== undefined ? formatNGN(service.price) : service.priceLabel ?? (service.priceMode === "referral" ? "Referral" : "Custom quote");
          const primaryLabel = isFuture ? "View / Request" : isRequest ? "Request Booking" : service.slug === "reset-wellness-smoothie-ticket" ? "Book & Pay ₦1,000" : selectedCategory === "Packages & Membership" ? "Select Package" : "Book Now";
