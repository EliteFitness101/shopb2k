...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "author", content: "ResoFit" }, { property: "og:title", content: "ResoFit — Africa's Personalized Wellness Platform" },
      { property: "og:description", content: "Personalized wellness, ResoFlex™ equipment and ChatB2K™ experiences." },
      { property: "og:type", content: "website" }, { property: "og:url", content: "https://www.resofit.fit/" },
      { name: "twitter:card", content: "summary" }, { name: "twitter:title", content: "ResoFit — Africa's Personalized Wellness Platform" },
      { name: "twitter:description", content: "Personalized wellness, ResoFlex™ equipment and ChatB2K™ experiences." },
    ],
    links: [{ rel: "stylesheet", href: appCss }, { rel: "canonical", href: "https://www.resofit.fit/" }],
  }),
...
function AppInner() {
  useCartSync(); const router = useRouter();
  useEffect(() => { captureAttributionFromUrl(); if (hasAnalyticsConsent()) initPixels(); const onConsent = () => { if (hasAnalyticsConsent()) initPixels(); }; window.addEventListener("resofit:consent-changed", onConsent); try { const raw = localStorage.getItem("resofit:imgPriority:v1"); if (raw) { const parsed = JSON.parse(raw) as { products?: Record<string, unknown> }; const ids = Object.keys(parsed.products ?? {}); if (ids.length) auditCatalog(ids); } } catch { /* noop */ } return () => window.removeEventListener("resofit:consent-changed", onConsent); }, []);
  useEffect(() => { const unsub = router.subscribe("onResolved", () => { if (hasAnalyticsConsent()) pixelPageView(window.location.pathname); }); return () => unsub(); }, [router]);
