...
async function fetchFeaturedProducts(): Promise<ShopifyProduct[]> {
  ...
  for (const q of tryQueries) {
    const res = await storefrontApiRequest<{ products: { edges: ShopifyProduct[] } }>(PRODUCTS_QUERY, { first: 8, query: q || undefined });
...
function Index() {
  const { data: featured } = useQuery({ queryKey: ["products", "homepage-featured"], queryFn: fetchFeaturedProducts, staleTime: 60_000 });
  useEffect(() => { trackEvent("landing_view"); }, []);
  useEffect(() => { if (!featured || featured.length === 0) return; preloadOnIdle(featured.slice(2, 6).map((p) => p.node.images.edges[0]?.node.url)); }, [featured]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0"><img src={heroImg} alt="Loaded olympic barbell in dramatic studio light" width={1536} height={1280} fetchPriority="high" decoding="async" className="h-full w-full object-cover opacity-70" /><div className="absolute inset-0 bg-gradient-to-r from-background via-background/70 to-transparent" /><div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent" /></div>
        <div className="relative mx-auto grid min-h-[88vh] max-w-7xl items-center px-6 py-24"><div className="max-w-2xl">
          ...
          <dl className="mt-16 grid max-w-md grid-cols-3 gap-6 border-t border-border/60 pt-8"><div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Reset</dt><dd className="mt-1 font-display text-3xl text-foreground">₦1k</dd></div><div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Warranty</dt><dd className="mt-1 font-display text-3xl text-foreground">Life</dd></div><div><dt className="text-xs uppercase tracking-widest text-muted-foreground">Ships</dt><dd className="mt-1 font-display text-3xl text-foreground">NG</dd></div></dl>
        </div></div>
      </section>
      <TrustBar />
