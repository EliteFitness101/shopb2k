...
function ProductPage() {
  ...
  return <div className="min-h-screen bg-background"><SiteHeader /><ProductDetail product={product} /><SiteFooter /></div>;
}
  ...
  return <article className="mx-auto max-w-7xl px-6 py-12">
    ...
    <div className="grid gap-12 lg:grid-cols-2">
      <div className="space-y-4">
        <div className="relative overflow-hidden rounded-sm border border-gold/20 bg-black/20">
          {product.mediaBackground?.video && <video className="absolute inset-0 h-full w-full object-cover opacity-35" autoPlay muted loop playsInline preload="metadata" poster={product.mediaBackground.image ?? images[0]?.url} aria-hidden="true"><source src={product.mediaBackground.video} type="video/mp4" /></video>}
          <div className="relative z-10"><ProductImage src={images[activeImg]?.url} alt={images[activeImg]?.altText} title={product.title} category={product.productType} productId={product.id} priority /></div>
        </div>
        {images.length > 1 && <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">{images.slice(0, 6).map((img, i) => <button key={img.url} type="button" onClick={() => setActiveImg(i)} aria-label={`View image ${i + 1}`} className={`overflow-hidden border ${i === activeImg ? "border-gold" : "border-border/60"}`}><ProductImage src={img.url} alt={img.altText} title={`${product.title} view ${i + 1}`} category={product.productType} tier={i === 0 ? "medium" : "low"} /></button>)}</div>}
      </div>
      <div>
