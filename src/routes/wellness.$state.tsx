      ...
      <main>
        <section className="border-b border-border/60">
          <div className="mx-auto max-w-7xl px-6 py-20 md:py-28">
            ...
            <a href={`/wellness/states/cities/hubs/geo-locator?state=${encodeURIComponent(state.slug)}`} className="mt-8 inline-flex items-center gap-2 bg-gold px-6 py-3 text-xs font-semibold uppercase tracking-widest text-gold-foreground">Open Wellness Locator <ArrowRight className="h-4 w-4" /></a>
          </div>
        </section>
        ...
        <section className="py-20">
          <div className="mx-auto max-w-7xl px-6">
            ...
            {loading ? <p className="text-sm text-muted-foreground">Loading the canonical location registry…</p> : cities.length === 0 ? <div className="border border-border/60 p-8"><MapPin className="h-5 w-5 text-gold" /><h3 className="mt-4 font-display text-2xl">Network coming to {state.name}.</h3><p className="mt-2 max-w-xl text-sm text-muted-foreground">This geographic destination is registered in ResoFit, but no city or verified partner records are currently published. We will never fabricate availability.</p></div> : <div className="grid gap-px bg-border/60 sm:grid-cols-2 lg:grid-cols-3">{cities.map((city) => <a key={city.id} href={`/wellness/states/cities/hubs/geo-locator?state=${encodeURIComponent(state.slug)}&city=${encodeURIComponent(city.slug)}`} className="bg-background p-6 transition-colors hover:bg-card hover:text-gold"><span className="block text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{city.latitude !== null ? "Geo registered" : "Location registered"}</span><span className="mt-3 block font-display text-2xl">{city.name}</span><span className="mt-2 block text-xs uppercase tracking-widest text-muted-foreground">Explore hubs →</span></a>)}</div>}
          </div>
        </section>
      </main>
      <SiteFooter />
