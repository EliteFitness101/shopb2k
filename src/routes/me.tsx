...
const GOALS = [
  ...
  { value: "reset", label: "Full reset & wellness", productName: "7-Day Nigerian Reset Protocol", title: "ResoFit 7-Day Nigerian Reset Protocol", cta: "Start My Reset" },
];
const ACTIVITIES = [
...
const FALLBACK_PRODUCTS: Record<string, { url: string; title: string; cta: string }> = {
  ...
  reset: { url: "https://shop.resofit.fit/product/7-day-nigerian-reset", title: "ResoFit 7-Day Nigerian Reset Protocol", cta: "Start My Reset" },
};
...
async function canonicalRecommendation(a: Answers): Promise<Recommendation> {
  ...
  try {
    ...
    return { ...local, title: entity.name || local.title, summary: entity.description || local.summary, url: canonicalUrl };
  } catch {
    return local;
  }
}
