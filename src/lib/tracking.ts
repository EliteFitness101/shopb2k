...
export type TrackEvent =
  ...
  | "identity_started" | "identity_created" | "chatb2k_handoff" | "play_home_view" | "game_selected"
...
async function emitCanonicalTelemetry(event: TrackEvent, payload: Record<string, unknown>) { const eventName = CANONICAL_PUBLIC_EVENT[event]; if (!eventName || typeof window === "undefined") return; const attr = getAttribution(); const anonymousId = getAnonId(); const params = new URLSearchParams(window.location.search); try { await supabase.functions.invoke("resofit-event-ingest", { body: { event_name: eventName, contract_version: "1.0", idempotency_key: `${anonymousId}:${event}:${Date.now()}:${crypto.randomUUID()}`, anonymous_id: anonymousId, rsid: attr.rsid ?? params.get("rsid"), funnel_origin: attr.funnel_origin ?? params.get("funnel_origin"), utm: Object.fromEntries(params.entries()), source_system: "resofit", payload: { original_event: event, page: window.location.pathname, device: getDevice(), landing_variant: getLandingVariant(), attribution: attr, ...payload } } }); } catch { void 0; } }
export function track(event: TrackEvent, payload: Record<string, unknown> = {}) { if (typeof window === "undefined") return; void emitCanonicalTelemetry(event, payload); try { pixelEvent(event, { value: typeof payload.value === "number" ? payload.value : undefined, currency: typeof payload.currency === "string" ? payload.currency : "NGN", content_ids: Array.isArray(payload.content_ids) ? payload.content_ids as string[] : typeof payload.product_id === "string" ? [payload.product_id] : undefined, content_name: typeof payload.product_title === "string" ? payload.product_title : undefined, content_type: "product", num_items: typeof payload.quantity === "number" ? payload.quantity : undefined }); } catch { void 0; } }
