...
export default async function handler(req,res){
 ...
 try{
  ...
  const insert=await sb("resofit_events",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify({event_name:topic,source_system:"shopify",adapter:"shopify-webhook",contract_version:"2026-07",occurred_at:now,created_at:now,utm:{},payload:{topic,shop_domain:shopDomain,webhook_id:webhookId,event_id:eventId,triggered_at:triggeredAt,order_id:orderId,shopify:payload},idempotency_key:webhookId,correlation_id:orderId||null})});
  if(insert.ok)return send(res,{ok:true,duplicate:false,webhook_id:webhookId});if(insert.status===409)return send(res,{ok:true,duplicate:true,webhook_id:webhookId});console.error("[Shopify webhook] persistence failure",insert.status,await insert.text().catch(()=>""));return send(res,{ok:false,error:"Failed to persist Shopify order event"},500)
