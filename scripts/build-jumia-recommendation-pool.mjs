...
const FEED_URL = process.env.JUMIA_FEED_URL ?? process.env.JUMIA_AUTHORIZED_FEED_URL;
const TARGET = 1000;
...
if (!FEED_URL) throw new Error("JUMIA_FEED_URL is required");
...
const score = (x) => x.priceNgn<=0||x.stockQty<=0 ? 0 : Number(((x.codEligible?20:0)+Math.max(0,Math.min(5,x.rating??0))*8+Math.min(15,Math.log10((x.reviewCount??0)+1)*7)+(x.imageUrl?8:0)+Math.min(8,Math.log10(x.stockQty+1)*4)).toFixed(2));
const qualified = (x) => x.priceNgn>0&&x.stockQty>0;
async function loadFeed(){const r=await fetch(FEED_URL,{signal:AbortSignal.timeout(30000),headers:{accept:"application/json"}});if(!r.ok)throw new Error(`Jumia feed failed: HTTP ${r.status}`);return r.json();}
