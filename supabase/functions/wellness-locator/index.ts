...
const json = (body: unknown, status = 200) => new Response(JSON.stringify(body), { status, headers: { ...corsHeaders, "Content-Type": "application/json", "Cache-Control": "public, max-age=30" } });
const db = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!, { auth: { persistSession: false } });
const haversineKm = (lat1: number, lon1: number, lat2: number, lon2: number) => { const r = 6371; const dLat = ((lat2-lat1)*Math.PI)/180; const dLon = ((lon2-lon1)*Math.PI)/180; const a = Math.sin(dLat/2)**2 + Math.cos((lat1*Math.PI)/180)*Math.cos((lat2*Math.PI)/180)*Math.sin(dLon/2)**2; return 2*r*Math.asin(Math.sqrt(a)); };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "states") {
      ...
      if (error) throw error; return json({ states: data ?? [] });
    }
    if (action === "cities") {
      const state = url.searchParams.get("state"); if (!state) return json({ error: "state_required" }, 400);
      ...
      if (stateError) throw stateError; if (!stateRow) return json({ cities: [] });
    ...
    const legacyIds = (wellness ?? []).map(h => h.id); let legacyServices: Array<Record<string, unknown>> = [];
    if (legacyIds.length) { const { data, error } = await db.from("resofit_wellness_hub_services").select("id,hub_id,service_entity_id,service_name,description,price,currency,duration_minutes,booking_method").in("hub_id", legacyIds).eq("status", "active").order("service_name"); if (error) throw error; legacyServices = data ?? []; }
    const servicesByLegacyHub = new Map<string, Array<Record<string, unknown>>>();
    for (const item of legacyServices) { const list = servicesByLegacyHub.get(String(item.hub_id)) ?? []; list.push(item); servicesByLegacyHub.set(String(item.hub_id), list); }
    ...
    return json({ query:{state,city,service:service||null,q:q||null,radius_km:radius}, results, count:results.length });
  } catch (error) { console.error("wellness-locator error", error); return json({ error:"internal_error" },500); }
});
